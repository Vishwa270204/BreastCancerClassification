...
.hbar-divider  { width:1px; height:38px; background:rgba(255,255,255,0.22); margin:0 10px; }
...
/* partition line between row1 and row2 */
.inp-divider {
    height: 1px;
    background: linear-gradient(90deg, transparent 0%, #7f1d1d 30%, #ef4444 50%, #7f1d1d 70%, transparent 100%);
    margin: 10px 0 12px 0;
}

/* ── Force ALL labels in input section dark-red text ── */
.inp-section label,
.inp-section .stSelectbox label,
.inp-section .stNumberInput label {
...
/* ── Number inputs: smaller, white bg ── */
.inp-section input[type="number"],
.inp-section input {
...
.inp-section input:focus {
    border-color: #ef4444 !important;
    outline: none !important;
}

/* Number stepper buttons */
.inp-section button[data-testid="stNumberInputStepDown"],
.inp-section button[data-testid="stNumberInputStepUp"] {
...
/* ── Selectbox: dark bg ── */
.inp-section [data-baseweb="select"] > div {
...
.inp-section [data-baseweb="select"] svg { fill: #fca5a5 !important; }
.inp-section [data-baseweb="select"] [data-baseweb="select-option"] {
    background: #3d0a0a !important;
    color: #fef2f2 !important;
}

/* ── Predict button ── */
.inp-section .stButton > button {
...
.inp-section .stButton > button:hover { opacity: 0.88 !important; }

/* ── Main content ── */
.main-zone { padding: 22px 36px 36px 36px; background: #f5eded; }
...
# ── INPUT SECTION ──
st.markdown('<div class="inp-section">', unsafe_allow_html=True)
st.markdown('<div class="inp-title">⚙️ Patient Input Parameters</div>', unsafe_allow_html=True)

# ROW 1 — Model + Numerical
r1a, r1b, r1c, r1d, r1e = st.columns([1.6, 1.2, 1.2, 1.1, 1.1])
with r1a: selected_model     = st.selectbox("🤖 Model",           MODEL_NAMES)
with r1b: survival_months    = st.number_input("Survival Months",  min_value=1, max_value=120, value=40, step=1)
with r1c: tumor_size         = st.number_input("Tumor Size (mm)",  min_value=1, max_value=200, value=25, step=1)
with r1d: reginol_node_pos   = st.number_input("Node Positive",    min_value=0, max_value=50,  value=2,  step=1)
with r1e: regional_node_exam = st.number_input("Node Examined",    min_value=1, max_value=60,  value=10, step=1)

# ── Partition line ──
st.markdown('<div class="inp-divider"></div>', unsafe_allow_html=True)

# ROW 2 — Categorical + Predict
r2a, r2b, r2c, r2d = st.columns([1.4, 1.4, 1.4, 0.9])
with r2a: estrogen     = st.selectbox("Estrogen Status",     ["Positive", "Negative"])
with r2b: progesterone = st.selectbox("Progesterone Status", ["Positive", "Negative"])
with r2c: a_stage      = st.selectbox("A Stage",             ["Regional", "Distant"])
with r2d:
    st.markdown("<div style='height:22px'></div>", unsafe_allow_html=True)
    predict_btn = st.button("🔍 Run Prediction", use_container_width=True)

st.markdown('</div>', unsafe_allow_html=True)  # close inp-section
