...
with c2: survival_months    = st.number_input("Survival Months",  min_value=1,  max_value=120,   step=1)
with c3: tumor_size         = st.number_input("Tumor Size (mm)",  min_value=1,  max_value=200,   step=1)
with c4: reginol_node_pos   = st.number_input("Node Positive",    min_value=0,  max_value=50, step=1)
with c5: regional_node_exam = st.number_input("Node Examined",    min_value=1,  max_value=60,  step=1)
